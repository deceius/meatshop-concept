...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('customer_category'), 'has-success': fields.customer_category && fields.customer_category.valid }">
    ...
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <div class="input-group ">
            <multiselect :disabled="form.status >= 1 || isReadOnly == 1" v-model="form.customer_category"  placeholder="{{ trans('brackets/admin-ui::admin.forms.select_options') }}" :options="['Direct', 'Walk-in', 'Trader']" :multiple="false" :preselect-first="true" open-direction="bottom"></multiselect>
        </div>
...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('customer'), 'has-success': fields.customer && fields.customer.valid }">
    ...
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <multiselect  :disabled="form.status >= 1 || isReadOnly == 1" v-model="form.customer" placeholder="{{ trans('brackets/admin-ui::admin.forms.select_options') }}" label="name" track-by="id" :options="{{ $customers->toJson() }}" :multiple="false" open-direction="bottom"></multiselect>
