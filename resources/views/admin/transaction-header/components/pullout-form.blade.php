...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('branch'), 'has-success': fields.branch && fields.branch.valid }">
    <label for="item" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">Source Branch</label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('delivery_branch'), 'has-success': fields.delivery_branch && fields.delivery_branch.valid }">
    <label for="item" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">Delivery Branch</label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <multiselect :disabled="form.id"  v-model="form.delivery_branch" placeholder="{{ trans('brackets/admin-ui::admin.forms.select_options') }}" label="name" track-by="id" :options="{{ $branches->toJson() }}" :multiple="false" open-direction="bottom"></multiselect>
        <div v-if="errors.has('delivery_branch')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('delivery_branch') }}</div>
    </div>
</div>
...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('remarks'), 'has-success': fields.remarks && fields.remarks.valid }">
    ...
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <div class="input-group ">
            <input type="text" :disabled="form.status >= 1 || isReadOnly == 1" v-model="form.remarks"  @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('remarks'), 'form-control-success': fields.remarks && fields.remarks.valid}" id="remarks" name="remarks" placeholder="{{ trans('admin.'.$transactionType.'.columns.remarks') }}">
        </div>
